...
# استيراد الموديلات (الملفات) اللي سويناها
...
# تحميل البيانات من الملف عند بدء تشغيل البوت
...
def handle_supplier_menu_buttons(message):
    ...
    if message.text == 'إضافة مجهز':
        ...
        supplier_handlers.handle_assign_shops_start(bot, message, user_states, get_admin_markup) # نحتاج نمرر get_admin_markup
...
def handle_get_new_supplier_code(message):
    supplier_handlers.get_new_supplier_code(bot, message, user_states) # ما نحتاج نمرر get_admin_markup هنا
...
def handle_get_new_supplier_wallet_url(message):
    supplier_handlers.get_new_supplier_wallet_url(bot, message, user_states, get_admin_markup) # نحتاج نمرر get_admin_markup
...
# معالج تسلسل إضافة محل جديد
@bot.message_handler(func=lambda message: user_states.get(message.chat.id, {}).get('state') == 'awaiting_shop_info' and message.from_user.id == ADMIN_ID)
def handle_get_shop_info(message):
    shop_handlers.get_shop_info(bot, message, user_states, get_admin_markup)
